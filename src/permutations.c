...
const uint8_t DES_K1P[28] = {
    57, 49, 41, 33, 25, 17, 9 , 1 , 58, 50, 42, 34, 26, 18,
    10, 2 , 59, 51, 43, 35, 27, 19, 11, 3 , 60, 52, 44, 36,
};

const uint8_t DES_K2P[28] = {
    63, 55, 47, 39, 31, 23, 15, 7 , 62, 54, 46, 38, 30, 22,
    14, 6 , 61, 53, 45, 37, 29, 21, 13, 5 , 28, 20, 12, 4 ,
};

const uint8_t DES_CP[48] = {
...
uint64_t des_expansion_permutation(uint32_t block32b) {
    ...
    for (uint8_t i = 0; i < 48; ++i) {
        block48b |= (uint64_t)((block32b >> (32 - DES_EP[i])) & 0x01) << (63 - i);
    }
    return block48b;
}
...
uint32_t des_substitutions(uint64_t block48b) {
    ...
    des_split_48bits_to_6bits(block48b, blocks6b);
    des_substitution_6bits_to_4bits(blocks6b, blocks4b);
    return des_join_4bits_to_32bits(blocks4b);
}
...
void des_substitution_6bits_to_4bits(uint8_t *blocks6b, uint8_t *blocks4b) {
    ...
    for (uint8_t i = 0, j = 0; i < 8; i += 2, ++j) {
        block2b = des_extreme_bits(blocks6b[i]);
        block4b = des_middle_bits(blocks6b[i]);
        blocks4b[j] = DES_SBOX[i][block2b][block4b];

        block2b = des_extreme_bits(blocks6b[i + 1]);
        block4b = des_middle_bits(blocks6b[i + 1]);
        blocks4b[j] = (blocks4b[j] << 4) | DES_SBOX[i + 1][block2b][block4b];
    }
}
...
uint32_t des_permutation(uint32_t block32b) {
    ...
    for (uint8_t i = 0; i < 32; ++i) {
        new_block32b |= ((block32b >> (32 - DES_P[i])) & 0x01) << (31 - i);
    }
    return new_block32b;
}
...
uint64_t des_initial_permutation(uint64_t block64b) {
    ...
    for (uint8_t i = 0; i < 64; ++i) {
        new_block64b |= ((block64b >> (64 - DES_IP[i])) & 0x01) << (63 - i);
    }
    return new_block64b;
}
...
uint64_t des_final_permutation(uint64_t block64b) {
    uint64_t new_block64b = 0;
    for (uint8_t i = 0; i < 64; ++i) {
        new_block64b |= ((block64b >> (64 - DES_FP[i])) & 0x01) << (63 - i);
    }
    return new_block64b;
}
